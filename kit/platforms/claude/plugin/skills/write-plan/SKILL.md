...
- A design doc from `/coding-agent-kit:brainstorm-feature`.
- A clear, unambiguous spec from the user.

If neither exists and the request is non-trivial, use
`/coding-agent-kit:brainstorm-feature` first.
...
The checkbox is mechanical. `/coding-agent-kit:implement-task` flips it when the
task is done.
...
Use `/coding-agent-kit:implement-task` to execute one task at a time. This is
the safer default when review checkpoints matter.

Use `/coding-agent-kit:implement-plan` only when the approved plan is clear and
the user wants all remaining unchecked tasks implemented in one run. After
either implementation mode, use `/coding-agent-kit:review-feature` for the
