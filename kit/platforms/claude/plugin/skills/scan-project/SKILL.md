...
description: >
  ...
  large changes. Read the repository, document its architecture, create Claude
  Code project guidance, and suggest useful repo-scoped skills.
---
...
4. Existing docs, `CONTRIBUTING.md`, and existing `CLAUDE.md`.
...
Create or update the repository `CLAUDE.md` with:
...
Do not create `.opencode/`, `.codex/`, or Antigravity config files. Do not edit `.claude/settings.json`, `.claude/settings.local.json`, or `.mcp.json` unless the user explicitly approves a project config change.

### Step 5 - Suggest Repo-Scoped Skills

Suggest `.claude/skills/<project-specific-name>/SKILL.md` only for repeated
...
2. Use `/coding-agent-kit:skill-creator`.
3. Place the skill under `.claude/skills/<project-specific-name>/SKILL.md`.
