...
`/coding-agent-kit:write-plan`, with tasks in this format:
...
use `/coding-agent-kit:implement-task` one task at a time.

If no plan exists and the request is still a broad goal, use
`/coding-agent-kit:brainstorm-feature` and `/coding-agent-kit:write-plan`
first.
...
- A reminder to run `/coding-agent-kit:review-feature` for the final feature
  review.

Do not run `/coding-agent-kit:review-feature` automatically.
