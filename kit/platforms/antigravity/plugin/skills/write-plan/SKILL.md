...
- A design doc from the `brainstorm-feature` skill from this plugin.
- A clear, unambiguous spec from the user.

If neither exists and the request is non-trivial, use
the `brainstorm-feature` skill from this plugin first.
...
The checkbox is mechanical. The `implement-task` skill from this plugin flips it when the
task is done.
...
Use the `implement-task` skill from this plugin to execute one task at a time.
This is the safer default when review checkpoints matter.

Use the `implement-plan` skill from this plugin only when the approved plan is
clear and the user wants all remaining unchecked tasks implemented in one run.
After either implementation mode, use the `review-feature` skill for the final
feature review.
