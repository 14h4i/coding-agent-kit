...
the `write-plan` skill from this plugin, with tasks in this format:
...
the `brainstorm-feature` and `write-plan` skills from this plugin first.
...
- Use the `implement-plan` skill from this plugin only when the user
