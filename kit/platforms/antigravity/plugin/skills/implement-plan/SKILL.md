...
Expect a plan file in `docs/plans/*-plan.md` created by the `write-plan` skill
from this plugin, with tasks in this format:
...
use the `implement-task` skill from this plugin one task at a time.

If no plan exists and the request is still a broad goal, use the
`brainstorm-feature` and `write-plan` skills from this plugin first.
...
- If every task is checked, report that nothing remains and suggest the
  `review-feature` skill from this plugin.
...
- A reminder to run the `review-feature` skill for the final feature review.

Do not run `review-feature` automatically.
