...
description: >
  ...
  "run all tasks", "continue end to end", or "/implement-plan". Implement
  tasks sequentially, verify each task, mark each task done, run final
...
use `coding-agent-implement-task` or `/implement` one task at a time.

If no plan exists and the request describes a goal rather than a specific
implementation plan, use `coding-agent-brainstorm-feature` and
`coding-agent-write-plan` first.
...
- If every task is checked, report that nothing remains and suggest `/review`.
...
- A reminder to run `/review` for the final feature review.

Do not run `coding-agent-review-feature` or `/review` automatically.
