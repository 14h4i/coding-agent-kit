...
description: >
  ...
  large changes. Read the repository, document its architecture, create Codex
  project guidance, and suggest useful repo-scoped skills.
---

<!-- CODING_AGENT_KIT_MANAGED version=1.0.0 -->
...
4. Existing docs, `CONTRIBUTING.md`, and existing `AGENTS.md`.
...
Create or update the repository `AGENTS.md` with:
...
Do not create `.opencode/` files. Do not create `.codex/config.toml` unless
the user explicitly approves a project config change.
...
2. Use `$coding-agent-skill-creator`.
